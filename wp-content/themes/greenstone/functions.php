 ...
 *  URL: greenstone_theme.com | @greenstone_theme
...
if (function_exists('add_theme_support'))
{
    ...
    load_theme_textdomain('greenstone_theme', get_template_directory() . '/languages');
}
...
function greenstone_theme_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('ScrollToPlugin', get_template_directory_uri() . '/js/lib/ScrollToPlugin.min.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('ScrollToPlugin'); // Enqueue it!

        wp_register_script('DrawSvgPlugin', get_template_directory_uri() . '/js/lib/DrawSVGPlugin.min.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('DrawSvgPlugin'); // Enqueue it!

        wp_register_script('ScrollMagic', get_template_directory_uri() . '/js/lib/ScrollMagic.min.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('ScrollMagic'); // Enqueue it!

        wp_register_script('ScrollMagicGsap', get_template_directory_uri() . '/js/lib/animation.gsap.min.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('ScrollMagicGsap'); // Enqueue it!

        wp_register_script('isotope', get_template_directory_uri() . '/js/lib/isotope.js', array(), '2.7.1'); // Modernizr
        ...
        wp_register_script('attrGSAP', get_template_directory_uri() . '/js/lib/AttrPlugin.min.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('attrGSAP'); // Enqueue it!

        wp_register_script('counterUp', get_template_directory_uri() . '/js/lib/jquery.counterup.js', array('jquery'), '2.7.1'); // Modernizr
        wp_enqueue_script('counterUp'); // Enqueue it!


        wp_register_script('waypoints', get_template_directory_uri() . '/js/lib/jquery.waypoints.min.js', array('jquery'), '2.7.1'); // Modernizr
        wp_enqueue_script('waypoints'); // Enqueue it!

        wp_register_script('SplitText', get_template_directory_uri() . '/js/lib/SplitText.js', array(), '2.7.1'); // Modernizr
        wp_enqueue_script('SplitText'); // Enqueue it!

        wp_register_script('greenstone_themescripts', get_template_directory_uri() . '/scripts.js', array('jquery'), '1.0.0'); // Custom scripts
        wp_enqueue_script('greenstone_themescripts'); // Enqueue it!
    }
}

// Load HTML5 Blank conditional scripts
function greenstone_theme_conditional_scripts()
{
...
function greenstone_theme_styles()
{
...
if (function_exists('register_sidebar'))
{
    ...
    register_sidebar(array(
        'name' => __('Widget Area 1', 'greenstone_theme'),
        'description' => __('Description for this widget-area...', 'greenstone_theme'),
        'id' => 'widget-area-1',
    ...
    register_sidebar(array(
        'name' => __('Widget Area 2', 'greenstone_theme'),
        'description' => __('Description for this widget-area...', 'greenstone_theme'),
        'id' => 'widget-area-2',
        'before_widget' => '<div id="%1$s" class="%2$s">',
        'after_widget' => '</div>',
...
{
    global $post;
    return '... <a class="view-article" href="' . get_permalink($post->ID) . '">' . __('View Article', 'greenstone_theme') . '</a>';
}
...
function greenstone_themegravatar ($avatar_defaults)
{
...
function greenstone_themecomments($comment, $args, $depth)
{
...
// Add Actions
add_action('init', 'greenstone_theme_header_scripts'); // Add Custom Scripts to wp_head
add_action('wp_print_scripts', 'greenstone_theme_conditional_scripts'); // Add Conditional Page Scripts
add_action('get_header', 'enable_threaded_comments'); // Enable Threaded Comments
add_action('wp_enqueue_scripts', 'greenstone_theme_styles'); // Add Theme Stylesheet
...
// Add Filters
add_filter('avatar_defaults', 'greenstone_themegravatar'); // Custom Gravatar in Settings > Discussion
add_filter('body_class', 'add_slug_to_body_class'); // Add slug to body class (Starkers build)
add_filter('widget_text', 'shortcode_unautop'); // Allow shortcodes in Dynamic Sidebar
...
function create_post_type_managed()
{
  register_taxonomy_for_object_type('category', 'greenstone_theme'); // Register Taxonomies for Category
  register_taxonomy_for_object_type('post_tag', 'greenstone_theme');
    register_post_type( 'managed',
        array(
            'labels' => array(
                'name' => ('Managed Property'),
                'singular_name' => ('Managed Property')
            ),
        'public' => true,
        'has_archive' => true,
        'rewrite' => array('slug' => 'managed'),
        'supports' => array('title','editor'),
...
function custom_enter_title( $input ) {
    ...
    if( is_admin() && 'Enter title here' == $input && 'property' == $post_type )
